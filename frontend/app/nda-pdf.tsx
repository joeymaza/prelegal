...
const buildClauses = (law: string, jur: string): Clause[] => [
  ...
  { title: "Proprietary Rights.", body: `The Disclosing Party retains all of its intellectual property and other rights in its Confidential Information, and its disclosure to the Receiving Party grants no license under such rights.` },
  { title: "Disclaimer.", body: `ALL CONFIDENTIAL INFORMATION IS PROVIDED "AS IS", WITH ALL FAULTS, AND WITHOUT WARRANTIES, INCLUDING THE IMPLIED WARRANTIES OF TITLE, MERCHANTABILITY, AND FITNESS FOR A PARTICULAR PURPOSE.` },
  { title: "Governing Law and Jurisdiction.", body: `This MNDA and all matters relating hereto are governed by, and construed in accordance with, the laws of the State of ${law}, without regard to the conflict of laws provisions of such state. Any legal suit, action, or proceeding relating to this MNDA must be instituted in the federal or state courts located in ${jur}. Each party irrevocably submits to the exclusive jurisdiction of such courts in any such suit, action, or proceeding.` },
  { title: "Equitable Relief.", body: `A breach of this MNDA may cause irreparable harm for which monetary damages are an insufficient remedy. Upon a breach of this MNDA, the Disclosing Party is entitled to seek appropriate equitable relief, including an injunction, in addition to its other remedies.` },
  { title: "General.", body: `Neither party has an obligation under this MNDA to disclose Confidential Information to the other or proceed with any proposed transaction. Neither party may assign this MNDA without the prior written consent of the other party, except that either party may assign this MNDA in connection with a merger, reorganization, acquisition, or other transfer of all or substantially all its assets or voting securities. Any assignment in violation of this Section is null and void. This MNDA will bind and inure to the benefit of each party's permitted successors and assigns. Waivers must be signed by the waiving party's authorized representative and cannot be implied from conduct. If any provision of this MNDA is held unenforceable, it will be limited to the minimum extent necessary so the rest of this MNDA remains in effect. This MNDA (including the Cover Page) constitutes the entire agreement of the parties with respect to its subject matter and supersedes all prior and contemporaneous understandings, agreements, representations, and warranties, whether written or oral, regarding such subject matter. This MNDA may only be amended, modified, waived, or supplemented by an agreement in writing signed by both parties. Notices, requests, and approvals under this MNDA must be sent in writing to the email or postal addresses on the Cover Page and are deemed delivered on receipt. This MNDA may be executed in counterparts, including electronic copies, each of which is deemed an original and which together form the same agreement.` },
];

const SIGNATURE_ROWS = ["Signature", "Print Name", "Title", "Company", "Notice Address", "Date"];

function NdaDocument({ data }: { data: NdaFormData }) {
  const p1 = data.party1Name.trim() || "[Party 1]";
  const p2 = data.party2Name.trim() || "[Party 2]";
  const law = data.governingLawState.trim() || "[Governing Law State]";
  const jur = data.jurisdiction.trim() || "[Jurisdiction]";
  const purpose = data.purpose.trim() || "[Purpose]";
  const effective = formatDate(data.effectiveDate);
  const mndaTerm =
  ...
  const confTerm =
    data.confTermMode === "expires"
      ...
      : "In perpetuity.";
  const modifications = data.modifications.trim() || "None.";
  const clauses = buildClauses(law, jur);
